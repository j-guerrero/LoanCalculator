...
namespace LoanLibrary
{
    public class LoanModel
    {
        ...
        public static decimal calculateMinimumPayment(LoanModel loan)
        {
            ...
            decimal periodicPayments = months * 12;
            decimal periodicInterestRate = interest / 12;
            double discountFactor = (Math.Pow(((1 + (double)periodicInterestRate) - 1), periodicPayments)) / Math.Pow(((double)periodicInterestRate * (1 + (double)periodicInterestRate)), periodicPayments);

            paymentAmount = total / (decimal)discountFactor;
